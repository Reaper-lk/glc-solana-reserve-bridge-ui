...
/**
 * Whether `address` is on the server-side operator allowlist
 * (`middleware.ts`'s `/admin/fund-reserve/operator-check` — see that
 * file's docs for why the check lives there and not in an `app/api/*`
 * route). This is a defense-in-depth layer on top of production's
 * separate reverse-proxy access control in front of `/admin/*` — not the
 * page's only gate.
 */
export function useOperatorAuthorization(
  address: string | null,
): UseQueryResult<OperatorAuthorization> {
  return useQuery({
    queryKey: ["admin", "fund-reserve", "operator-check", address],
    enabled: Boolean(address),
    // Never cached across a different address — re-checked fresh every
    // time the connected wallet changes, since this is an authorization
    // decision, not a display value.
    staleTime: 0,
    queryFn: async () => {
      const response = await fetch(
        `/admin/fund-reserve/operator-check?address=${encodeURIComponent(address ?? "")}`,
      );
      if (!response.ok) {
        throw new Error("Could not verify operator authorization");
      }
      const body: unknown = await response.json();
      if (
        typeof body !== "object" ||
        body === null ||
        typeof (body as { authorized?: unknown }).authorized !== "boolean"
      ) {
        throw new Error("Operator authorization check returned a malformed response");
      }
      return { authorized: (body as { authorized: boolean }).authorized };
    },
  });
}
