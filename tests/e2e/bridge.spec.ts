...
test.describe("bridge form", () => {
  ...
  test("requests a live quote and shows the exact 1% gross/fee/net breakdown", async ({
    page,
  }) => {
    await page.goto("/bridge");
    ...
    await expect(page.getByText("Bridge fee (1%)")).toBeVisible();
    await expect(page.getByText("You receive")).toBeVisible();
    await expect(page.getByText(/1000\.00000000/)).toBeVisible();
    await expect(page.getByText(/−10\.00000000/)).toBeVisible();
    await expect(page.getByText(/990\.00000000/)).toBeVisible();
  });
